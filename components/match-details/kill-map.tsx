...
function getKillColor(kill: Kill, isVictim: boolean = false): string {
...
function adjustCoordinatesForMap(
  ...
  imageHeight: number,
  kill?: Kill,
  isVictim: boolean = false, // Indica si estamos ajustando posición de víctima o atacante
  ...
  if (needsRecalculation) {
    ...
    const hasValidGamePos = gamePos && 
      typeof gamePos.x === 'number' && !isNaN(gamePos.x) && isFinite(gamePos.x) &&
      typeof gamePos.y === 'number' && !isNaN(gamePos.y) && isFinite(gamePos.y)
    ...
    if (hasValidGamePos && gamePos) {
      ...
      if (kill && kill.id <= 3) {
        ...
        console.log(`[${normalizedName} Transform] Kill ${kill.id} ${isVictim ? '(victim)' : '(attacker)'} (${kill.position}):`, {
    ...
    const isProblematicCoords = normalizedName === "de_nuke" && 
      ((x > 900 && x < 1000) || (y > 900 && y < 1000))
    
    if (!isProblematicCoords && x > 0 && x < imageWidth && y > 0 && y < imageHeight) {
      // Las coordenadas de imagen parecen válidas, usarlas directamente
      console.warn(`[${normalizedName} Transform] Kill ${kill?.id || 'unknown'} ${isVictim ? '(victim)' : '(attacker)'}: Usando coordenadas de imagen directamente (no hay coordenadas del juego)`, { x, y })
      return { x, y }
    }
    
    // Si las coordenadas son problemáticas y no tenemos coordenadas del juego, loggear y usar fallback
    console.warn(`[${normalizedName} Transform] Kill ${kill?.id || 'unknown'} ${isVictim ? '(victim)' : '(attacker)'}: Sin coordenadas del juego válidas, usando fallback`, {
      imageCoords: { x, y },
  ...
  useEffect(() => {
    ...
    killsWithCoordinates.forEach((kill) => {
      ...
      if (showAttackerPositions && kill.attackerImagePosition) {
        ...
        const adjusted = adjustCoordinatesForMap(mapName, x, y, imageDimensions.width, imageDimensions.height, kill, false, mapConfig.mapData)
        x = adjusted.x
        y = adjusted.y
      ...
      if (showVictimPositions && kill.victimImagePosition) {
        ...
        // Aplicar corrección específica del mapa (ej: Inferno)
        const adjusted = adjustCoordinatesForMap(mapName, x, y, imageDimensions.width, imageDimensions.height, kill, true, mapConfig.mapData)
        x = adjusted.x
        y = adjusted.y
        // Escalar coordenadas al tamaño del canvas
        const scaledX = x * scaleX
    ...
    killsWithCoordinates.forEach((kill) => {
      if (showAttackerPositions && kill.attackerImagePosition) {
        ...
        const distance = Math.sqrt(
          Math.pow(imageX - adjusted.x, 2) + 
          Math.pow(imageY - adjusted.y, 2)
        )
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            ...
            <div>
              <p className="text-2xl font-bold text-green-400">{killsWithCoordinates.filter(k => k.isGoodPlay).length}</p>
              <p className="text-sm text-gray-300">Buenas Jugadas</p>
            </div>
            <div>
              <p className="text-2xl font-bold text-red-400">{killsWithCoordinates.filter(k => !k.isGoodPlay).length}</p>
